...
contract BatchRepayBadDebtStewardBaseTest is Test {
    ...
    function setUp() public {
        ...
        steward = new BatchRepayBadDebtSteward(
        ...
        for (uint256 i = 0; i < usersWithBadDebt.length; i++) {
            uint256 currentDebtAmount = IERC20(assetDebtToken).balanceOf(
                usersWithBadDebt[i]
            );
        ...
        for (uint256 i = 0; i < usersEligibleForLiquidations.length; i++) {
            uint256 currentDebtAmount = IERC20(assetDebtToken).balanceOf(
                usersEligibleForLiquidations[i]
            );
            ...
            DataTypes.ReserveDataLegacy
                memory collateralReserveData = AaveV3Avalanche
                    .POOL
                    .getReserveData(collateralsEligibleForLiquidations[i]);
            uint256 collateralBalance = IERC20(
                collateralReserveData.aTokenAddress
            ).balanceOf(usersEligibleForLiquidations[i]);
...
contract BatchRepayBadDebtStewardTest is BatchRepayBadDebtStewardBaseTest {
    function test_batchRepayBadDebt() public {
        ...
        uint256 collectorBalanceBefore = IERC20(assetUnderlying).balanceOf(
            collector
        );
        ...
        uint256 collectorBalanceAfter = IERC20(assetUnderlying).balanceOf(
            collector
        );
    ...
    function test_reverts_batchRepayBadDebt_caller_not_cleaner(
        address caller
    ) public {
    ...
    function test_batchLiquidate() public {
        ...
        uint256 stewardBalanceBefore = IERC20(assetUnderlying).balanceOf(
            address(steward)
        );

        vm.prank(guardian);
        steward.batchLiquidate(
            assetUnderlying,
            collateralsEligibleForLiquidations,
            usersEligibleForLiquidations
        );

        uint256 collectorBalanceAfter = IERC20(assetUnderlying).balanceOf(
            collector
        );
        uint256 stewardBalanceAfter = IERC20(assetUnderlying).balanceOf(
            address(steward)
        );

        assertTrue(collectorBalanceBefore >= collectorBalanceAfter);
        assertTrue(
            collectorBalanceBefore - collectorBalanceAfter <=
    ...
    function test_batchLiquidateWithMaxCap() public {
        ...
        assertTrue(collectorBalanceBefore >= collectorBalanceAfter);
        assertTrue(
            collectorBalanceBefore - collectorBalanceAfter <= passedAmount
        );
    ...
    function test_reverts_batchLiquidate_caller_not_cleaner(
        address caller
    ) public {
        vm.assume(caller != guardian);
    ...
    function test_getDebtAmount() public view {
        (
            uint256 receivedTotalDebt,
            uint256[] memory receivedUsersDebtAmounts
        ) = steward.getDebtAmount(
                assetUnderlying,
                usersEligibleForLiquidations
            );
        ...
        assertEq(
            receivedUsersDebtAmounts.length,
            usersEligibleForLiquidations.length
        );

        for (uint256 i = 0; i < usersEligibleForLiquidations.length; i++) {
            assertEq(
                receivedUsersDebtAmounts[i],
                usersEligibleForLiquidationsDebtAmounts[i]
            );
        }
    }
    ...
    function test_getBadDebtAmount() public view {
        (
            uint256 receivedTotalBadDebt,
            uint256[] memory receivedUsersBadDebtAmounts
        ) = steward.getBadDebtAmount(assetUnderlying, usersWithBadDebt);

        assertEq(receivedTotalBadDebt, totalBadDebt);
        assertEq(
            receivedUsersBadDebtAmounts.length,
            usersBadDebtAmounts.length
        );
