...
contract ConstructorTest is RewardsStewardTest {
    ...
    function test_successful() public {
        ...
        assertEq(
            address(newSteward.REWARDS_CONTROLLER()),
            AaveV3Ethereum.DEFAULT_INCENTIVES_CONTROLLER
        );
    }
}
...
contract ClaimAllRewardsTest is RewardsStewardTest {
    ...
    function test_successful() public {
        uint256 balanceBefore = IERC20(AaveV3EthereumAssets.USDS_A_TOKEN)
        ...
        assertEq(
            IERC20(AaveV3EthereumAssets.USDS_A_TOKEN).balanceOf(
                address(AaveV3Ethereum.COLLECTOR)
            ),
            balanceBefore + amounts[3]
        );
    }
}
